...
class CosineAnnealingWarmRestarts(_LRScheduler):
    def __init__(self, optimizer, num_epochs, iters_per_epoch=0, last_epoch = -1, t_actual = 11, t_mul = 2, lr_max = 0.01 , lr_min = 0.0001, decay = 1):
        ...
        self.cur_iter = 0
        self.iters_per_epoch = iters_per_epoch
        #self.iterations_per_batch = iters_per_epoch #math.ceil(self.total_images/self.batch_size)
        self.cycle = 0
        self.ti = self.iters_per_epoch * self.t_actual
        super(CosineAnnealingWarmRestarts, self).__init__(optimizer, last_epoch)
    
    def get_lr(self):

        self.cur_iter %= self.ti
        cosine_value = math.cos((self.cur_iter / self.ti) * math.pi)
        new_lr = self.lr_min + (0.5 * (self.lr_max - self.lr_min) * (1+cosine_value))
        self.cur_iter += 1
        
        if self.cur_iter==self.ti:
            import time
            time.sleep(5)
            print("Warm restart")
            self.ti *= self.t_mul
